...
This version adds a reflecting boundary inside the tolerance radius atol.
...
class RKF78reflect:
    def __init__(self,  x0, t0, rfun, step_max, atol, step_min=1e-8, max_iter=0, verbose=0):
        ...
        this class implements a reflecting boundary at the origin. 
        For fixed timestep, the particle will be reflected when it is about to cross the origin.
        For adaptive timestep, the particle will be reflected if it approaches within atol distance to the origin, or if it has just crossed the origin for the first time 
            ...
            verbose: print reflecting events when verbose > 0.
        ...
        if x0[0]<0:
                raise RuntimeError("must start from a positive initial position when using reflecting boundary")
    ...
    def step1(self):
        ...
        if self.fixstep:           #definite step method
            ...
            xnew=self.x+np.dot([34./105,9./35,9./35,9./280,9./280,41./840,41./840],[k5,k6,k7,k8,k9,k11,k12])*self.h
            if (self.x[1]>0 and xnew[0]<0): #crossing point, mirror current position
                self.reflect()
            else:
                self.x=xnew
                self.t+=self.h
            ...
            if delta<=1:            #error under control
                ...
                if self.h<=self.hmin:
                    ...
                    self.step1()
            if self.x[0]<self.atol[0] and self.x[1]<0:#approaching 0 within atol, mirror it
                self.reflect()
                
    
    def reflect(self):
        '''reflect the particle back with an sign-inverted velocity'''
        if self.verbose:
            print('reflect at t=%.2e, x=%.2e, dt=%.2e'%(self.t, self.x[0], -self.x[0]/self.x[1]))
        self.x[1]=-self.x[1]
        self.t+=self.x[0]/self.x[1] #a rough timescale estimate
